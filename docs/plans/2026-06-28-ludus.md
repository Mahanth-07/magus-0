...
- **Planner mapping:** the spec's `MagusPlanner` component (game-agnostic: screenshot+objective+legal actions+outcomes+rules → `Decision`) is fulfilled by `ModelProvider.decide(PlannerContext) → Decision` plus `build_prompt`. We deliberately do **not** add a redundant `MagusPlanner` wrapper class — the provider *is* the planner. `PlannerContext` is the planner's input contract.
...
- [ ] **Step 3: Write `README.md`** — what Magus-0 is, the shared-planner thesis, the verified baseline-vs-memory numbers, the InsForge integration (Postgres traces + S3 screenshots + gateway vision + realtime), how to run, and the architecture diagram from the spec.
